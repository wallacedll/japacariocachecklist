...
const LoginPage = ({ onLogin, onGoToRegister, onGoToForgot, theme, onToggleTheme }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const profiles = await db.query("profiles", "id,name,email,role,phone,sector_id,unit_id,created_at,sectors(name)", { id: `eq.${authData.user.id}` });
      ...
      const user = {
        ...
        name: profile.name,
        role: profile.role,
        sector: profile.sectors?.name || "Gerência",
...
const MainApp = ({ user, unit, onLogout, theme, onToggleTheme }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Load templates with items and responsible person
        const tplData = await db.query(
          "checklist_templates",
          "id,title,moment,schedule,frequency,active,sector_id,responsible_id,sectors(name),profiles!checklist_templates_responsible_id_fkey(name),template_items(id,text,type,required,photo_required,unit,min_value,max_value,sort_order)",
        ...
        const formattedTemplates = tplData.map(t => ({
          id: t.id,
          title: t.title,
          sector: t.sectors?.name || "Gerência",
          moment: t.moment,
          active: t.active,
          responsible: t.profiles?.name || "Não atribuído",
        ...
        const execData = await db.query(
          "executions",
          "id,template_id,date,scheduled_time,started_at,completed_at,status,completion_rate,is_late,signature,sector_id,responsible_id,sectors(name),profiles!executions_responsible_id_fkey(name),checklist_templates(title)",
        ...
        const formattedExecs = execData.map(e => ({
          ...
          templateTitle: e.checklist_templates?.title || "",
          sector: e.sectors?.name || "",
          responsible: e.profiles?.name || "",
          date: e.date,
        ...
        // Load all users for team page
        const usersData = await db.query(
          "profiles",
          "id,name,email,role,phone,active,created_at,sector_id,sectors(name)",
          { "unit_id": `eq.${unit.id}` }
        );
        setAllUsers(usersData.map(u => ({
          ...u,
          sector: u.sectors?.name || "Gerência",
